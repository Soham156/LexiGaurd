  ...
  return (
    <div className="flex flex-col h-[60vh]">
      ...
      <div className="flex-1 overflow-auto p-2 space-y-2">
        {messages.map((m, i) => (
          <div key={i} className={`p-2 rounded ${m.from === 'bot' ? 'bg-gray-100 self-start' : 'bg-sky-600 text-white self-end'}`}>
            {m.text}
          </div>
        ))}
      </div>
      ...
      <div className="mt-2 flex gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} className="flex-1 border rounded p-2" placeholder="Ask about this document..." />
